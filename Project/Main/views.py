...
def user_login(request):
    top = Profile.objects.all().order_by(
        "-points")[:min(Profile.objects.count(), 5)]
        ...
        if 'email' in request.POST:
            ...
            if password == conf_password:
                if User.objects.filter(email=email).exists():
                    messages.error(
                        request, "An account already exists on this email.")
...
def Question_detail(request, id):
    ...
    queryset_list = Answer.objects.all().filter(
        question=instance).order_by("-accepted", "-votes", "-timestamp")
...
def profile(request, id):
    ...
    queryset_list = Question.objects.all().filter(
        user=profile).order_by("-timestamp")
    paginator = Paginator(queryset_list, 10)
    page = request.GET.get('page')
    username = 'Login'
    if User.is_active:
    ...
    queryset_list1 = Answer.objects.all().filter(
        user=profile).order_by("-timestamp")
...
def Update_pro(request, id):
    ...
    if request.user == instance.user:
        form = Update(request.POST or None,
